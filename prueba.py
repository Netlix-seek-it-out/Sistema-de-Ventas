...
logo_original = tk.PhotoImage(file="ventacore.gif")
logo = logo_original.subsample(10, 10)
...
nombre_app = tk.Label(menu_frame,  text="¡Venta Core!", bg="#1e293b", width=18, height=2, font=("Arial", 12, "bold"), fg="#7A68EE", cursor="hand2")
nombre_app.grid(row=0, column=0, padx=10, pady=8)
...
nav_frame.pack(fill="x", pady=(0,10), ipady=3)
...
historial_boton_barra = tk.Label(menu_frame, text="📄 Historial", bg="#1e293b", width=18, height=2, font=("Arial", 12, "bold"), cursor="hand2", fg="white")
historial_boton_barra.grid(row=0, column=4, padx=10, pady=8)
historial_boton_barra.bind("<Button-1>", lambda e: mostrar_historial())
...
card_window = historial_canvas.create_window(
    (0, 0),
    window=card,
    anchor="nw"
)
...
def ajustar_ancho(event):
    historial_canvas.itemconfigure(card_window, width=event.width)

historial_canvas.bind("<Configure>", ajustar_ancho)
